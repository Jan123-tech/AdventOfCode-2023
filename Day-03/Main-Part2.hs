...
main = do
    ...
    let matches = map (\(p, _) -> map (pointsMap !) $ filter (`Map.member` pointsMap) p) allPointsToCheck0
    let uniqueMaches = map (map head . group . sort . map snd) matches
    let uniqueMatchProducts = [product x | x <- uniqueMaches, 2 == length x]
    let total = sum uniqueMatchProducts
...
getSymbols (rowIndex, row) = do
    ...
    let symbols = map (\(i, x) -> ((rowIndex, i), x)) $ filter (\(_, x) -> x == '*') lineWithIndex
    symbols